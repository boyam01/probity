...
|---|---|---|---|
| Probity is a methodology and local harness for testing AI coding-agent success claims. | Supported | README, `INTERFACE_CONTRACT.md`, `probity/runner.py`, `probity/checker.py` | "Agent reliability methodology for false-green testing of AI coding agents." |
| Built-in checker -> stats -> verdict path is zero-LLM. | Supported for built-in path | `probity/`, `probity/verdict.py`, `probity/checker.py` | "No LLM on the built-in verdict path." |
...
| Probity uses repeated trials and Wilson intervals. | Supported, not novel | `probity/stats.py`, `docs/RELATED_WORK.md` | "Uses repeated trials and Wilson intervals, common statistical infrastructure." |
| PASS means no falsification found under the registered battery. | Supported boundary | `probity/verdict.py`, README, methodology docs | "PASS is absence of found falsification, not proof of correctness." |
...
artifacts are not part of this public repo; the README and docs summarize the
general method only.
